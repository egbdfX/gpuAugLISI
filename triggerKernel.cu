...
__global__ void split_auglisi(float* data_1, float* data_2, float* result, size_t size, size_t unit_size, size_t ima, size_t unit_num) {
    ...
    for (size_t fac = 1; fac <= factor;fac = fac + 1){
		if (tid+(fac-1)*1024 < unit_size*unit_size){
			if (fac == 1) {
				...
				sharedNumDen[tid+1024] = 0; // Denominator
			}
...
int splitaug(float* input_data_1, float* input_data_2, float* result_array, size_t size, size_t unit_size, size_t ima) {
    ...
    split_auglisi<<<num_blocks,num_threads,shared_mem_size>>>(d_data_1, d_data_2, result_data, size, unit_size, ima, unit_num);
    cudaStatus = cudaDeviceSynchronize();
    if (cudaStatus != cudaSuccess) {
